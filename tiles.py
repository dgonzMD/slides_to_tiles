...
def score_tiles(slide_num, np_img=None, dimensions=None, small_tile_in_tile=False):
  ...
  tiles_by_row_column = tile_sum.tiles_by_row_column()
  rank = 0
  for t in tiles_by_row_column:
    rank += 1
    t.rank = rank
...
class TileSummary:
  ...
  def tiles_by_row_column(self):
    """
      ...
      Returns:
         List of the tiles ranked by score.
    """
    sorted_list = sorted(self.tiles, key=lambda t: (t.r, t.c), reverse=False)
    return sorted_list
  ...
  def top_tiles(self):
    ...
    sorted_tiles = self.tiles_by_row_column()
    top_tiles = [t for t in  sorted_tiles if t.tissue_percentage>5]
    return top_tiles
